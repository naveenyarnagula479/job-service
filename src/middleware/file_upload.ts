...
export const uploadFiles = multer({
  ...
  fileFilter: (req, file, cb) => {
    console.log('fileFilter.. file:'+nodeUtil.inspect(file))
    if (file != null && (file.mimetype == 'text/plain' )) {
      cb(null, true)
    } else {
      cb(null, false)
      const err: Error = new Error('Only .txt format allowed!')
      if(file == null){
        err.message = 'File is INVALID'
      }
      err.name = ERROR_FILE_FILTER
...
export const noteFileReader = function (fieldName: string): any {
  return function (req: any, res, next): any {
    uploadFiles.single(fieldName)(req, res, function (err) {
      if (!err) {
        // Everything went fine.
        next()
      } else if (err instanceof multer.MulterError) {
        ...
        if (err.name === ERROR_FILE_FILTER){
          ...
          // An unknown error occurred when uploading.
          logger.error(`${TAG}.noteFileReader(): Unknown error.`, err)
          res.status(500).send({ error: 'Technical Issues' })
        }
      }
    })
